...
- 미팅 raw transcript ingest 금지.
  미팅에서 나온 결정·요구사항·이슈는 의미에 맞는 raw 폴더에 정리.
...
- **메타데이터**: 모든 wiki/ 페이지 상단에 frontmatter:
  ...
  ---
  ```
  - 기존 페이지를 단순 갱신할 때는 `last_updated` 만 갱신.
...
- `specs/` — 요구사항·설계 문서
- `incidents/` — 장애·이슈 회고
- `references/` — 외부 학습 자료와 그 자료를 바탕으로 한 학습 메모.
  하위에 `articles/`, `courses/`, `papers/`, `books/`

### wiki/ (LLM 합성, 개인 검토)

- `index.md` — 전체 목차. 매 ingest 후 갱신.
- `log.md` — ingest/lint 이력. 매 작업 후 1 행 추가.
- `overview.md` — 나중에 다시 볼 때 쓰는 한 페이지 요약.
...
- **Canonical gap**: raw 에 자주 등장하지만 canonical wiki 페이지가 없는 개념·컴포넌트·결정.
...
예시:
```markdown
## [2026-04-29] ingest | raw/references/courses/20260428-langgraph-state.md
...
- 변경 후 `git diff` 로 raw/ 가 수정되지 않았는지 확인.
- 필요하면 의미 단위로 커밋한다.
- 외부 공유 전에는 비밀·자격증명·PII 포함 여부를 다시 확인한다.
