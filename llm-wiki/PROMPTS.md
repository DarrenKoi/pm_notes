...
> 로컬 PC 에서 개인 위키를 관리할 때 쓰는 프롬프트. 복사 → 경로·날짜만 치환 → 사용.
...
새 프로젝트에 starter 폴더를 복사한 직후 1 회 실행.
...
2. 영향받는 wiki/ 페이지 식별 — 기존 갱신 + 신규 생성.
   ...
   - raw 폴더별 기본 ingest 대상:
     · raw/journals/                      → wiki/components/, wiki/concepts/,
                                             wiki/decisions/ 중 내용 기준
     · raw/decisions/                     → wiki/decisions/
     · raw/specs/                         → wiki/components/, wiki/runbooks/, wiki/overview.md
   ...
   - frontmatter 필수 (SCHEMA §2).
   - 기존 페이지를 수정하는 경우 last_updated 만 오늘 날짜로 갱신.
...
- Canonical gap: raw/ 에 자주 나오지만 canonical wiki 페이지가 없는 개념·컴포넌트·결정

산출:
- <WIKI_ROOT>/wiki/log.md 최상단에 lint 항목 prepend:
  ## [<오늘 날짜>] lint | wiki-wide
- 각 발견 사항을 카테고리별 bullet 으로 나열, 영향받는 페이지 경로 포함.
- **자동 수정 금지.** 결과를 보고 직접 수정 범위를 결정.
- 발견 0 건이면 "all clean" 으로 명시 (lint 가 부실한 게 아니라면 OK 신호).
```

---

## 7. Change Summary — 로컬 변경 요약

작업 후 변경사항을 짧게 정리할 때 사용.

```text
방금 작업한 위키 변경사항을 요약해줘.

형식:
- Summary: 한 줄 요약
- Changed files: 생성/수정된 wiki 페이지 목록
- Citations: 새로 추가된 raw/ 또는 src/ 인용
- Open questions: 발견된 미해결 사항 (없으면 "없음")
- Checks: raw/ 수정 여부, 비밀·PII 포함 여부, index/log 갱신 여부
```
...
- 변경 후 `git diff` 로 확인하고 필요할 때만 커밋한다.
...
| [`02-ingest-raw.md`](./prompts_example/02-ingest-raw.md) | raw 자료 합성 (의사결정, 스펙, 참고 자료) | §2 |
