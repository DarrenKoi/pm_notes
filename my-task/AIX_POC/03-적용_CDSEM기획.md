...
L1 Mega Process : 계측(MI) 운영
  L2 Process Group : CD-SEM Recipe Management
    L3 Process : Recipe Creation & Operation
      L4 Activity : [A] 의뢰 접수·검토 → [B] Recipe 셋업(①~⑦) → [C] 양산 Run → [D] 모니터링·이상대응
        L5 Task (AI 배치 후보) : B-③ 패턴 등록 · B-④ 좌표·박스 · B-⑥ 오인식 수정 (핵심)
                                · D-① 실시간 측정 이상탐지 (차기) · A-2 의뢰서 부족정보 플래깅 (보조)
```

> L3을 기존 "Recipe Creation"에서 **"Creation & Operation"** 으로 넓혀, 셋업 이후 양산 Run·모니터링까지 한 흐름으로 본다(범위는 엔지니어 확인 필요 항목).
...
Recipe Creation 셋업만이 아니라 **의뢰 접수부터 양산 Run·모니터링까지** 전체 흐름을 펼친다(엔지니어 확인: [B] 셋업 ①~⑦은 확정, [A]·[C]·[D]의 세부 단계·경계는 확인 필요 항목).
...
[C] 양산 Run (실행)
  C1 양산 로트 측정 실행 (등록 Recipe로 자동 측정)
  C2 측정 결과 수집·1차 판정 (정상/이상)

[D] 모니터링·이상 대응
  D1 측정 안정성 모니터링 (재현성·드리프트·장비 상태)            ❗ 공백 (사람 주기점검·야간/주말 의존)
...
- **[C] Run**: 등록 Recipe로 자동 실행되어 *수작업 비중이 낮음* → 자동화 후보 우선순위 낮음(단, 셋업 부실 시 오측 유발).
...
| C Run | 셋업 부실 시 오측 유발 (Run 자체 수작업은 적음) | 🔵 낮음 | 자동화 여지 적음 |
...
| 양산 Run 실행 | C-1 | — | 低 | 低 | (이미 자동) | — | 제외 *(자동화 여지 적음)* |
...
- 🔭 **차기 범위**: C 양산 Run·D 모니터링은 1차 범위 밖 — D-1 실시간 이상탐지를 차기 후보로 둔다.
