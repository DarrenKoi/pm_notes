...
| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 1 | [Airflow 기본 개념](./01-basic-concepts.md) | DAG, Task, Operator, Scheduler, Worker, XCom, Connection |
| 2 | [첫 번째 DAG 만들기](./02-first-dag-python-file.md) | 기존 Python 파일을 BashOperator/PythonOperator로 실행 |
| 3 | [의존성, 스케줄, 재시도](./03-dependencies-scheduling-retry.md) | `task1 >> task2`, `schedule`, `catchup`, retry, timeout |
| 4 | [데이터와 상태 관리](./04-data-and-state.md) | XCom, 파일 저장소, 멱등성, 날짜 파티션, secret 관리 |
...
| 역할 | 보통 가능한 일 |
|------|---------------|
| 일반 사용자 | DAG 업로드, 수동 실행, 로그 확인, Variable/Connection 일부 사용 |
