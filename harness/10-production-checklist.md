...
> 01~09의 기본 내용과 11의 최신 적용 판단을 배포 전 점검표로 압축했다. "반드시"는 없으면 배포하지 않을
...
- [ ] 실행마다 트레이스(모델 입출력 또는 통제된 원문 참조, 도구 호출, 토큰,
      종료 이유)가 남고 마스킹·접근 권한·보존 기간이 정해져 있다
- [ ] 트레이스에 하네스 버전이 기록된다
...
아래는 [11](./11-current-trends.md)의 사례를 운영에 옮길 때의 권고다.
해당 기능을 사용하지 않으면 적용 제외 사유를 남긴다.

| 도입 기능 | 배포 전에 확인할 것 |
|---|---|
| 도구 검색 | 필수 도구 검색 성공, 호출 직전 권한 재검증 |
| 코드 기반 도구 호출 | 개별 쓰기 승인·감사 유지, 부분 실패 표시 |
| 컨텍스트 압축 | 목표·승인 범위·미완료 검증·부수효과 기록 보존 |
| fork 워커 | 필요한 정보만 상속, 별도 권한·전체 예산 적용 |
| MCP 개정·Tasks | 코어·확장·SDK 버전, 추가 입력·취소·단절 복구 확인 |
| 장기 실행·외부 쓰기 | 응답 유실 시 중복 실행 없이 결과 조회 |
| 자동 하네스 개선 | 수정 범위 제한, 판정기·최종 평가 셋 보호 |

외부 쓰기가 있는 시스템의 멱등성·승인 재개는 첫 달로 미룰 항목이 아니라 배포 전
조건이다. 긴 작업이 제품 요구사항이면 체크포인트 복구도 배포 전에 검증한다.

완료 증거에는 다음을 남긴다.

- 모델·하네스·도구·평가 셋·실행 환경 버전
- 성공 수/전체 시행 수, 비용·지연, 안전 회귀 결과
- 테스트 명령과 결과, 실제로 검증한 환경
- 미검증 항목과 적용 제외 사유, 롤백할 버전

“로컬 가짜 도구 통과”와 “사내 대상 시스템 통합 검증 통과”를 구분한다.
브라우저·외부 쓰기를 실제로 실행하지 않았다면 해당 검증은 미실행으로 표시한다.
