...
| 등급 | 예 | 기본 정책 |
|---|---|---|
| read-only | 파일 읽기, 로그 검색 | 접근 권한·조회 범위·호출 한도 안에서 허용 |
...
- **MCP(Model Context Protocol)**: 도구를 하네스와 분리된 서버로 표준화한다.
  여러 에이전트가 같은 도구를 공유할 수 있다. 도구 정의를 전부 모델에 넣는지는
  하네스의 공개 정책에 달려 있다. 검색·지연 로딩을 쓰면 필요한 정의만 넣는다.
  MCP 연결과 모델 컨텍스트 주입은 별도 단계다.
...
- JSON Schema로 인자를 검증하고, 실패하면 예외 대신 교정 메시지를 돌려준다
  (운영 구현에서 추가해야 한다. [02](./02-agent-loop.md)의 `execute`는
  JSON 파싱과 호출 예외 처리만 하며 완전한 Schema 검증기는 아니다)
...
Anthropic은 Tool Search와 Programmatic Tool Calling을 발표했다.
전자는 **정의의 크기**, 후자는 **중간 결과와 모델 왕복 횟수**를 줄이는 접근이다.
[공식 발표, 2025-11-24](https://www.anthropic.com/engineering/advanced-tool-use)

다음은 이 노트의 적용 예다.

```text
도구 검색: “문서 검색” 요청 → 권한 내 카탈로그 검색 → 관련 도구 명세 공개
코드 처리: 로그 페이지 조회 → 시간 필터 → 오류별 집계 → 집계와 원문 참조 반환
```

원시 로그 10만 행을 모델이 세도록 하지 않고 코드가 집계하게 한다.
실패한 페이지를 조용히 제외하면 “오류 0건”이라는 잘못된 결론이 나올 수 있다.
반환값에 `processed`, `failed`, `truncated`, 원문 참조를 함께 남긴다.

도구 검색은 권한 부여가 아니다. 발견한 도구도 실행 직전에 사용자·대상·인자를
검증한다. 코드에서 호출하더라도 개별 쓰기 도구의 승인과 감사를 통과해야 한다.
스킬 역시 지침·스크립트를 공급하는 수단이며 실행 권한을 스스로 부여하지 않는다.

### 운영 도구의 계약

| 항목 | 예시 또는 판단 기준 |
|---|---|
| 입력 제약 | 시간 범위, 최대 행 수, 경로 허용 범위, 추가 인자 거부 |
| 결과 상태 | 성공·빈 결과·부분 성공·거부·일시 오류 구분 |
| 출력 한도 | 페이지 크기, 다음 커서, 잘림과 원본 참조 |
| 부수효과 | 읽기·쓰기·실행 분류, 중복 호출 의미 |
| 중단 의미 | 타임아웃 후 작업이 계속되는지, 취소 확인 방법 |
| 호환성 | 도구 스키마·서버·프로토콜 버전 |

위 `format_page`는 이미 메모리에 있는 목록을 나누는 학습 예제다. 대량 데이터는
서버에서 페이지를 제한한다. 커서는 가능하면 불투명 토큰으로 반환하고,
음수 커서나 0 이하 limit은 실행 전에 검증한다.
MCP의 최신 버전 차이는 [11](./11-current-trends.md)에서 다룬다.
