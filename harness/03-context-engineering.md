...
요약용 LLM 호출이 필요 없다. 동일 버전으로 다시 가져올 수 있는 읽기 결과를
대상으로 한다. 아래 예제는 도구 종류를 구분하지 않으므로 운영에서는 하단의
보존 계약에 따라 제거 대상을 제한한다.
...
다음은 다음 행동에 필요한 증거를 보존하기 위한 이 노트의 권고 계약이다.

| 구분 | 압축 뒤에도 보존할 내용 |
|---|---|
| 목표·범위 | 최신 요구, 제외 대상, 수정 가능한 경로 |
| 권한 | 승인된 구체 행동·범위, 거절된 행동; 권한 원본은 별도 기록 |
| 진행 | 완료·진행 중·미착수, 검증 명령과 실제 결과 |
| 부수효과 | 생성된 외부 ID, 진행 중 호출, 멱등성 키 |
| 근거 | 파일·페이지·버전, 다시 읽을 원문 위치 |
| 불확실성 | 가설과 확인된 사실, 검증하지 못한 대상 |

“표 추출 완료” 대신 “30개 중 27개 통과, 병합 셀 3개 실패,
결과는 임시 경로에 있으며 최종 반영 전”이라고 남긴다. 검증하지 않은 결과를
요약 과정에서 완료 상태로 바꾸면 다음 세션은 검증을 건너뛴다.

위 `clear_old_tool_results`는 단순 치환 예제다. 운영에서는 다시 조회 가능한
읽기 결과만 제거 대상으로 선택한다. 외부 쓰기 결과·승인·실패 증거는 별도
저장하고 참조를 남긴다. 시간에 따라 바뀌는 검색 결과는 동일 질의를 다시 해도
같은 증거가 아니므로 원문 버전이나 스냅샷이 필요하다.

압축 결과는 새로운 사용자 지시가 아니라 이전 작업에서 유래한 데이터로 취급한다.
원문의 악성 지시가 요약을 거쳐 신뢰된 지시로 승격되지 않게 하고,
권한 판정은 별도의 정책·승인 기록을 조회한다.

### 캐시와 도구 검색은 함께 설계한다

작은 도구 집합에서는 prefix를 유지하고, 도구가 많으면 검색으로 필요한 정의만
추가하는 방식도 비교한다. Anthropic은 지연 로딩한 도구를 검색 뒤 추가해
고정 prefix의 캐시를 유지하는 방식을 설명한다.
[공식 도구 검색 설명](https://www.anthropic.com/engineering/advanced-tool-use)

fork도 캐시를 활용할 수 있지만 시스템 프롬프트 변경·압축 시점·서버 구현에 따라
효과가 달라진다. 캐시 적중률만 높이려고 불필요한 이력을 보존하지 말고
성공률·입력 토큰·지연을 함께 본다.
[컨텍스트 모드 사례](https://www.langchain.com/blog/organizing-context-in-a-multi-agent-harness)
